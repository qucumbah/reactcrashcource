...
class Set extends React.Component {
  ...
  render() {
    let words = this.props.set.words.map((wordObject, index)=>{
      ...
      return <Word word={word} translation={translation} key={word+translation} onEdition={(...args)=>this.handleWordEdition(index, ...args)} onRemoval={()=>this.handleWordRemoval(index)} />
    });
    return (
